function [lengthList,freqList,timeList,alist,gaborInfo,header] = getBurstLengthMP_hue(analogData,trialIDs,timeVals,thresholdFactor,displayFlag,stimulusPeriodS,baselinePeriodS,burstFreqRangeHz,maxIteration,adaptiveDictionaryParam,dictionarySize,gaborInfo,header)
...
if isempty(gaborInfo)
    ...
    [gaborInfo,header] = getStochasticDictionaryMP3p1_parallel(analogData1,timeVals,maxIteration,adaptiveDictionaryParam,dictionarySize);
end

if ~exist('trialIDs','var')
    if ~isempty(analogData)
        assert( size(analogData,1)==size(gaborInfo,1), 'Size mismatch netween analogData and gaborInfo; Supply full analogData for electrode')
    end
    
    trialIDs = 1:size(gaborInfo,1);
    
end
numTrials = numel(trialIDs);

%%%%%%%%%%%%%%%%%%%%%%%%%%% Threshold Computation %%%%%%%%%%%%%%%%%%%%%%%%%
% threshold = thresholdFactor*getMeanBaseline(gaborInfo(trialIDs,:,:),timeVals(1),baselinePeriodS,burstFreqRangeHz);
...
for i=1:numTrials
    x = squeeze(gaborInfo(trialIDs(i),:,:));
    o = x(:,SCALE);
    f = x(:,FREQ);
    a = x(:,AMPLI);
    t = x(:,POS)+timeVals(1);
...
if displayFlag == 1
    ...
    subplot(221);
    [meanE,freqVals]=getEnergyMP3p1(gaborInfo(trialIDs,:,:),header,timeVals);
    ...
    for i=1:numTrials
        disp(['Trial: ' num2str(trialIDs(i))]);
        
        subplot(222);
        cla;
        plot(timeVals,analogData(trialIDs(i),:));
        xlim(timeRange);

        subplot(224);
        cla;
        E = mp2tf(squeeze(gaborInfo(trialIDs(i),:,:)),header(trialIDs(i),:));
